...
data = spark.sql("select p.product_name, array_join(collect_list(c.category_name), ', ') categories from products_view p left join pc_view pc on p.product_id = pc.product_id left join cat_view c on pc.cat_id = c.cat_id group by p.product_name order by p.product_name")
data.show()
